...
const OPERATING_SYSTEM_MODULES: OSModule[] = [
  {
    ...
    color: "from-[#FFD700]/20 to-[#B8860B]/20"
  },
  {
    ...
    color: "from-[#0a0e27]/40 to-[#E5E4E2]/10"
  },
  {
    ...
    color: "from-[#E5E4E2]/10 to-[#B76E79]/20"
  },
  {
    ...
    color: "from-[#FFD700]/10 to-[#020202]/40"
  },
  {
...
const SystemCard = ({ module, index }: { module: OSModule, index: number }) => (
  <motion.div
    ...
    className="group relative p-8 rounded-[2rem] bg-[#0a0e27]/40 border border-white/5 backdrop-blur-xl overflow-hidden hover:border-[#FFD700]/30 hover:shadow-[0_0_30px_rgba(255,215,0,0.15)] transition-all"
  >
    ...
    <div className="relative z-10">
      <div className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center mb-6 text-[#E5E4E2] group-hover:text-[#FFD700] transition-colors">
        {module.icon}
      </div>
      <h3 className="text-xl font-bold mb-3 tracking-tight text-white group-hover:text-[#FFD700] transition-colors">{module.title}</h3>
      <p className="text-[#E5E4E2]/70 text-sm leading-relaxed font-light">
        {module.description}
      </p>
    </div>
  </motion.div>
...
export default function NextSection(): JSX.Element {
  ...
  const [clients, setClients] = useState<TestimonialData[]>(TESTIMONIALS);

  useEffect(() => {
    fetch("/data/clientsContent.json")
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <section ref={containerRef} className="relative bg-transparent text-white pt-20 pb-32 px-6 lg:px-24 overflow-hidden selection:bg-[#FFD700]/30 z-10">
      ...
      {/* SECTION HEADER */}
      <div id="modules" className="max-w-4xl mb-24 relative z-10 pt-10">
        <motion.span initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} className="inline-block text-[#FFD700] font-mono text-xs uppercase tracking-[0.4em] mb-4">
          System Design
        </motion.span>
        <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-5xl lg:text-7xl font-black tracking-tighter mb-8 italic uppercase leading-none font-serif text-white">
          A Scalable Remote <br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FFD700] to-[#B8860B]">IT Engine.</span>
        </motion.h2>
      </div>
      ...
      <div className="relative py-24 border-y border-white/5 bg-[#0a0e27]/40 rounded-[3rem] px-8 overflow-hidden mb-40 backdrop-blur-md">
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-5 gap-8 lg:gap-12 z-10">
          <div className="hidden md:block absolute top-[28px] left-[10%] right-[10%] h-[2px] bg-gradient-to-r from-[#FFD700]/20 via-white/10 to-[#B8860B]/20" />
          {PIPELINE_STEPS.map((step, idx) => (
            <motion.div key={`step-${idx}`} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: idx * 0.15 }} className="relative flex flex-col items-center text-center group">
              <div className="w-14 h-14 rounded-full bg-[#020202]/80 border border-white/20 flex items-center justify-center mb-6 z-10 transition-all group-hover:border-[#FFD700] group-hover:shadow-[0_0_20px_rgba(255,215,0,0.3)] backdrop-blur-sm">
                <span className="text-xs font-bold font-mono text-zinc-500 group-hover:text-[#FFD700]">0{idx + 1}</span>
              </div>
      ...
      <div id="deployments" className="mb-40 relative z-10 pt-20 mt-10">
        ...
        <div className="relative h-[500px] md:h-[400px] flex items-center justify-center overflow-visible">
          {clients.map((testimonial, i) => {
            const isCenter = i === activeIndex;
            const isLeft = i === (activeIndex - 1 + clients.length) % clients.length;
            const isRight = i === (activeIndex + 1) % clients.length;
            ...
            return (
              <motion.div
                ...
                className="absolute w-full max-w-xl p-8 lg:p-12 rounded-[2.5rem] bg-[#0a0e27]/40 border border-white/10 backdrop-blur-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)]"
              >
                <div
                ...
                <div className="flex flex-col gap-6 relative z-10">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-mono text-[#FFD700] uppercase">{testimonial.role}</p>
                    </div>
                  </div>
                  <p className="text-lg text-[#E5E4E2] italic font-light">&ldquo;{testimonial.content}&rdquo;</p>
                  <div className="flex gap-1">
                    {[...Array(5)].map((_, star) => <Star key={star} className="w-3 h-3 fill-[#FFD700] text-[#FFD700]" />)}
                  </div>
                </div>
              </motion.div>
            );
        ...
        <div className="flex justify-center items-center gap-12 mt-12 relative z-10">
          <button onClick={prevTestimonial} className="group p-4 rounded-full border border-white/10 bg-[#0a0e27]/50 hover:border-[#FFD700] hover:text-[#FFD700] transition-all active:scale-90 backdrop-blur-sm">
            <ArrowLeft className="w-6 h-6" />
          </button>
          <div className="flex gap-3">
            {clients.map((_, i) => (
              <div key={i} className={`h-1.5 transition-all duration-500 rounded-full ${activeIndex === i ? "w-10 bg-[#FFD700]" : "w-2 bg-zinc-800"}`} />
            ))}
          </div>
          <button onClick={nextTestimonial} className="group p-4 rounded-full border border-white/10 bg-[#0a0e27]/50 hover:border-[#FFD700] hover:text-[#FFD700] transition-all active:scale-90 backdrop-blur-sm">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-40 relative z-10">
        {STATS.map((stat, idx) => (
          <motion.div key={`stat-${idx}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }} className="p-8 rounded-3xl bg-white/5 border border-white/10 text-center group backdrop-blur-md hover:border-[#FFD700]/30 transition-colors">
            <span className="text-4xl lg:text-5xl font-black mb-2 text-white group-hover:text-[#FFD700] transition-colors block">{stat.value}</span>
            <span className="text-[10px] uppercase tracking-[0.3em] text-[#E5E4E2]/50 font-bold block">{stat.label}</span>
          </motion.div>
        ))}
      </div>

      {/* SYSTEMS WE BUILD GRID */}
      <div className="mb-32 relative z-10 pt-20">
        <div className="text-center mb-16">
          <h2 className="text-[#FFD700] font-mono text-xs uppercase tracking-[0.4em] mb-4">Engineering Output</h2>
          <h3 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter mb-6 text-white font-serif">
            Systems We Build.
          </h3>
          <p className="text-[#E5E4E2]/70 font-light max-w-2xl mx-auto">
            From highly scalable SaaS platforms to intelligent multimodal agents, we architect and deploy sophisticated systems designed for production environments.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {CAPABILITIES.map((cap, idx) => (
            <motion.div 
              ...
              className="p-8 md:p-10 rounded-[2rem] bg-white/5 border border-white/10 hover:border-[#FFD700]/30 hover:bg-[#0a0e27]/40 transition-all group backdrop-blur-sm"
            >
              <div className="w-14 h-14 rounded-2xl bg-[#020202]/80 border border-white/20 flex items-center justify-center mb-6 group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(255,215,0,0.2)] transition-all">
                {cap.icon}
              </div>
              <h4 className="text-xl font-bold text-white mb-3 group-hover:text-[#FFD700] transition-colors">{cap.title}</h4>
              <p className="text-[#E5E4E2]/70 text-sm leading-relaxed font-light">{cap.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <motion.div id="onboarding" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="flex flex-col items-center justify-center text-center relative z-10 pt-20">
        ...
        <Link href="/services">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="flex items-center gap-4 bg-white text-[#0a0e27] px-10 py-5 rounded-full font-black uppercase text-sm tracking-widest shadow-[0_0_20px_rgba(255,215,0,0.15)] hover:shadow-[0_0_40px_rgba(255,215,0,0.4)] transition-all group"
          >
            Initiate Onboarding <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </motion.button>
        </Link>
      </motion.div>
    </section>
