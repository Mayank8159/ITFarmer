...
const TESTIMONIALS: TestimonialData[] = [
  {
    ...
    content: "The precision of their Talent Engine is unmatched. We assembled a 10-man specialized squad in under 72 hours. It feels like having an internal elite team, minus the overhead.",
  ...
  },
  {
    ...
    content: "Scaling our infrastructure was becoming a nightmare until we integrated with their OS. The transparency in their Pipeline Visualization gave us the confidence to ship 30% faster.",
  ...
  },
  {
    ...
    content: "Military-grade is an understatement. Their Security Layer caught vulnerabilities our previous vendors missed. This is the gold standard for distributed IT operations.",
  ...
  return (
    <section ref={containerRef} className="relative bg-[#020202] text-white py-32 px-6 lg:px-24 overflow-hidden selection:bg-blue-500/30">
      ...
      <div className="max-w-4xl mb-24">
        ...
        <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-5xl lg:text-7xl font-black tracking-tighter mb-8 italic uppercase">
      ...
      <div className="relative py-24 border-y border-white/5 bg-zinc-900/10 rounded-[3rem] px-8 overflow-hidden mb-40">
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-5 gap-8 lg:gap-12">
          ...
          {PIPELINE_STEPS.map((step, idx) => (
            <motion.div key={idx} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: idx * 0.15 }} className="relative flex flex-col items-center text-center group">
      ...
      <div className="mb-40 relative">
        ...
        <div className="flex justify-center items-center gap-12 mt-12">
          <button 
            ...
            className="group p-4 rounded-full border border-white/10 bg-zinc-900/50 hover:border-blue-500 hover:text-blue-500 transition-all active:scale-90"
          >
          ...
          <div className="flex gap-3">
            {TESTIMONIALS.map((_, i) => (
              <div 
                key={i} 
                className={`h-1.5 transition-all duration-500 rounded-full ${activeIndex === i ? "w-10 bg-blue-500" : "w-2 bg-zinc-800"}`} 
              />
            ))}
          </div>
