...
export default function AuthPage() {
  // Pulling 'login' and 'isLoggedIn' from your updated context
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        if (isLogin) {
          // SUCCESS: Pass both Token and Username to AuthContext
          // We use the username from the form payload
          login(data.access_token, payload.username as string);
          
          // Small delay for state to settle before redirecting
          setTimeout(() => {
  ...
  // If already logged in, show a simple redirect message or auto-redirect
  if (isLoggedIn) {
    return (
